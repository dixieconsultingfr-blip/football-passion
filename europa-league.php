...
$matchsEL  = array_values(array_filter($matchsAll, fn($m) => ($m['competition'] ?? '') === 'Europa'));

// ── Derniers résultats : la dernière date jouée ──
$termines = array_values(array_filter($matchsEL, fn($m) => ($m['status'] ?? '') === 'FINISHED'));
...
$articlesEL  = array_values(array_filter($articlesAll, fn($a) =>
    ($a['categorie'] ?? '') === 'Europa' || in_array('Europa League', $a['etiquettes'] ?? [])
));
usort($articlesEL, fn($a, $b) => strtotime($b['date']) <=> strtotime($a['date']));

$page_title = 'Europa League — Résultats et actualités 2026-2027';
$meta_desc  = "Résultats et calendrier de la Ligue Europa 2026-2027 : qualifications, phase de ligue, actualités.";
...
<script type="application/ld+json">
{
  ...
  "sport": "Football",
  "url": "https://football-passion.fr/europa-league.php"
}
</script>
...
<header class="mb-10 flex items-center gap-4">
    <span class="inline-flex items-center justify-center w-14 h-14 bg-white rounded-xl p-2 shrink-0">
        <img src="/images/logo/logo-europa-league.webp" alt="Logo Europa League" class="w-full h-full object-contain" />
    </span>
    <div>
        <p class="text-green-400 text-xs font-semibold uppercase tracking-widest mb-1">Coupe d'Europe</p>
        <h1 class="text-4xl font-bold text-white mb-1">Europa League</h1>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <section class="bg-gray-900/40 border border-gray-800 rounded-2xl p-5 sm:p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="/archives.php?comp=Europa&amp;saison=2026-2027" class="text-green-400 hover:text-green-300 text-xs font-semibold shrink-0">Historique de la saison →</a>
            </div>
    ...
    <div class="lg:col-span-1 space-y-6">
        ...
        <section class="bg-gray-900/40 border border-gray-800 rounded-2xl p-5">
            ...
            <?php else: ?>
            <div class="space-y-3">
                <?php foreach (array_slice($articlesEL, 0, 6) as $a): ?>
            ...
            <a href="/blog?cat=Europa" class="block text-center text-green-400 hover:text-green-300 text-xs font-semibold mt-3">Tous les articles Europa League →</a>
            <?php endif; ?>
        </section>
