...
$enDirect = array_values(array_filter($matchsL2, fn($m) => ($m['status'] ?? '') === 'LIVE'));
$aVenir = array_values(array_filter($matchsL2, fn($m) => !in_array($m['status'] ?? 'SCHEDULED', ['FINISHED', 'LIVE'], true)));
