...
$matchsL3  = array_values(array_filter($matchsAll, fn($m) => ($m['competition'] ?? '') === 'L3'));
...
$termines = array_filter($matchsL3, fn($m) => ($m['status'] ?? '') === 'FINISHED');
$table = [];
foreach ($matchsL3 as $m) {
...
$enDirect = array_values(array_filter($matchsL3, fn($m) => ($m['status'] ?? '') === 'LIVE'));
$aVenir = array_values(array_filter($matchsL3, fn($m) => !in_array($m['status'] ?? 'SCHEDULED', ['FINISHED', 'LIVE'], true)));
...
$articlesL3  = array_values(array_filter($articlesAll, fn($a) => ($a['categorie'] ?? '') === 'L3'));
usort($articlesL3, fn($a, $b) => strtotime($b['date']) <=> strtotime($a['date']));

$page_title = 'Ligue 3 Betclic — Classement, résultats et actualités 2026-2027';
$meta_desc  = "Classement, résultats et actualités de la Ligue 3 Betclic 2026-2027, nouveau 3e niveau du football français : calendrier, clubs et infos diffusion.";
...
<script type="application/ld+json">
{
  ...
  "@type": "SportsEvent",
  "name": "Ligue 3 Betclic 2026-2027",
  "sport": "Football",
  "url": "https://football-passion.fr/ligue-3.php"
}
</script>
...
<nav class="text-xs text-gray-400 mb-8 flex items-center gap-2">
    ...
    <span>›</span>
    <span class="text-gray-400">Ligue 3</span>
</nav>
...
<header class="mb-10 flex items-center gap-4">
    ...
    <div>
        ...
        <h1 class="text-4xl font-bold text-white mb-1">Ligue 3 Betclic</h1>
        <p class="text-gray-400 text-sm">Saison 2026-2027 · Première saison sous ce nom, classement, résultats et actualités</p>
    </div>
</header>

<!-- Bloc présentation nouveau championnat -->
<section class="bg-green-900/10 border border-green-800 rounded-xl p-5 mb-8">
    <p class="text-gray-300 text-sm leading-relaxed">
        La <strong class="text-white">Ligue 3 Betclic</strong> remplace officiellement le National à partir de la saison 2026-2027 et devient le 3e niveau professionnel du football français, avec 18 clubs. Nouveauté marquante : <strong class="text-white">Ligue 1+</strong> diffuse en exclusivité l'intégralité des 9 matchs de chaque journée. Retrouvez ici le calendrier, les résultats et le classement du championnat.
    </p>
</section>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <section class="bg-gray-900/40 border border-gray-800 rounded-2xl p-5 sm:p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="/archives.php?comp=L3&amp;saison=2026-2027" class="text-green-400 hover:text-green-300 text-xs font-semibold shrink-0">Historique de la saison →</a>
            </div>
    ...
    <div class="lg:col-span-1 space-y-6">
        ...
        <section class="bg-gray-900/40 border border-gray-800 rounded-2xl p-5">
            ...
            <?php else: ?>
            <div class="space-y-3">
                <?php foreach (array_slice($articlesL3, 0, 4) as $a): ?>
            ...
            <a href="/blog?cat=L3" class="block text-center text-green-400 hover:text-green-300 text-xs font-semibold mt-3">Tous les articles L3 →</a>
            <?php endif; ?>
        </section>
...
<section class="flex flex-wrap gap-4 justify-center pt-8 pb-4">
    ...
    <a href="/droits-tv-football.php" class="border border-green-700 text-green-400 hover:bg-green-700 hover:text-white px-5 py-2 text-sm font-semibold rounded transition-colors">📺 Où regarder la Ligue 3 ?</a>
</section>
