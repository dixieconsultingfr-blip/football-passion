...
### 3.69 Fiches joueurs équipe de France, statut Euro 2028 dynamique — 5 septembre 2026

Nouvelle fonctionnalité demandée par l'utilisateur : une fiche par joueur de l'équipe de France, liée dynamiquement à son statut de sélection pour les qualifications Euro 2028 puis pour le tournoi final. Décision explicite de l'utilisateur : démarrer avec un seul joueur (Kylian Mbappé) et rendre le statut réellement dynamique (mis à jour à chaque annonce officielle), plutôt qu'une mention statique.

**Architecture** : nouveau fichier `data/joueurs.json` (tableau simple, pas de découpage en fichiers séparés vu le faible volume attendu ~23-26 joueurs), avec un champ `statut_euro2028` structuré (`convoque_qualifs`, `convoque_finale`, tous deux nullable — `null` = pas encore annoncé, `true`/`false` une fois la liste connue — plus `date_maj`). Fonctions `load_joueurs()` / `load_joueur()` ajoutées à `blog/helpers.php`, sur le modèle exact de `load_articles_index()` / `load_article()`. Nouveau template `joueur.php?slug=...` avec des badges visuels distincts pour chaque état (✅ convoqué / ➖ non retenu / ⏳ en attente d'annonce). Section "Les joueurs des Bleus" ajoutée sur `equipe-france.php`, listant les fiches disponibles. Page ajoutée au sitemap (boucle sur `data/joueurs.json`, même logique que les articles).
